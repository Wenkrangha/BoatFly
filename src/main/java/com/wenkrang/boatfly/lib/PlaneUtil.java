...
public class PlaneUtil {
    ...
    //但现在已经不怎么用了
    //所有的Tag都在玩家乘上的时候添加
    public static void getPlane(@NotNull Location location) {
        final var boat = (Boat) Objects.requireNonNull(location.getWorld())
                .spawnEntity(location, EntityType.valueOf(
    ...
    //货运飞船的tag
    public static void getPlaneLevel2(@NotNull Location location) {
        final var boat = (Boat) Objects.requireNonNull(location.getWorld())
                .spawnEntity(location, EntityType.valueOf(
                        VersionChecker.isVersionBelow("1.21.2") ? "BOAT" : "OAK_BOAT"
                ));
    ...
    //客运飞船的tag
    public static void getPlaneLevel3(@NotNull Location location) {
        final var boat = (Boat) Objects.requireNonNull(location.getWorld())
                .spawnEntity(location, EntityType.valueOf(
                        VersionChecker.isVersionBelow("1.21.2") ? "BOAT" : "OAK_BOAT"
                ));
